...
This section discusses how the requirements were developed, and how using this user stories, a user interface mockup and an ER diagram for the system were developed.
...
\autoref{fig:mockup1} shows the initial prototype for the design, which displayed visualisations relating to the project on the main page.  A project was chosen from a drop-down menu and all the tickets related to that project were listed below.   Each ticket consisted of the graphs, metadata and the associated conversation.
...
The Notification API specification \cite{site:mdnNotifications} is currently under development, however the technology is supported by most of the modern browsers, as shown in \autoref{table:notification}. Notifications consist of four main attributes: title, body, icon and tag (i.e. an identifier for the notification).
...
\end{tabular}
\caption{Support for web notifications API}
...
This feature is implemented through a small button \autoref{fig:saveMessage} located in the bottom right corner of each message. If pressed, the message will be saved as an item of metadata in the information tab as shown on \autoref{fig:messageSaved}.
...
With this feature, the user can see a list of all the relevant information in a chat and thus there is no need to read the entire conversation to understand what is happening. Thereby, there is no duplication of effort and every relevant content can be saved as the conversation is occurring.
