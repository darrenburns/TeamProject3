...
\textbf{Research} \autoref{research}
...
\subsection{Conversation focused}
...
%----------------------------------------------------------------------------
\section{Learning to solve the problem}
\label{requirements}
...
It was noted that the products that were looked at often had native apps and websites designed for desktop, but very few employed a responsive design to cater to their mobile users. It was decided that the design of the project must provide for the variety of devices the user may use to navigate the site using responsive design.
...
For comparison, the lack of feature branch integration displayed during the early stages of the project is displayed below. The diagram \ref{figure:diagram2} shows branching activity between the 24th and 30th of November 2014.
...
Implementation began with the creation of a framework from which to build upon.  Models were implemented as identified in the ER diagram \autoref{figure:ERDiagram} using Django’s built in Object Relational Mapper (ORM). During the initial stages of development, these objects were mapped to a SQLite schema, but as development progressed, the PostgreSQL \cite{site:postgresql} database management system was used instead. PostgreSQL was used due to the ease by which it can be connected to a Heroku hosted application, whilst SQLite is only suitable for desktop or mobile applications. An additional factor contributing to this decision is that SQLite \cite{site:sqlite} does not scale well on web applications due to the fact that concurrent access rates can change unpredictably, and are often much higher in comparison to client side applications.
...
This JSON object can then be parsed using Javascript and injected into the Document Object Model (DOM) using Ajax, therefore making it visible to the user without requiring a page refresh.
...
Attach metadata is a core feature of the system. Metadata provides essential information for developers and manager regarding a conversation. There is an attach data button located at the right edge of the screen, when it is clicked, a dropdown menu is displayed with the six metadata types that a user can edit.
...
\item \textit{Cost} \par
It is a decimal field which is flexible to development teams because is does not enforce a specific method for cost prediction. Therefore, teams can use methods ranging from Function Point Analysis to Planning Poker.
\item \textit{Due Date} \par
The day and time which the issue discussed in the conversation should be solved.
\item \textit{Notes} \par
It is a textarea field which is primarily used for linking resources of the conversation. This field supports markdown notation meaning that teams can also use it in the way they prefer, by just applying a common structure to the note.
\item \textit{Priority} \par
This metadata type consists of a list with three values Low, Medium and High. Conversations marked as high priority receive a red background in the conversations list.
\item \textit{Tags} \par
Users can set tags for a conversation. Every tag has a special label colour and is displayed at the side of conversation title to make it easy to identify what is the content of the conversation.
\end{itemize}
...
Although the graphs provided cannot provide an absolute indication of the exact state of the project, the may provide hints to project managers of issues regarding progress or levels of developer contribution. A conversation which has been open for a week without any activity may indicate that an issue is being ignored, and the visualisations intend to aid in identifying situations like this. 

The graphs were constructed using the D3 \cite{site:d3} Javascript library. D3 allowed for the creation of graphs with relative ease, powered by the conversation data from Firebase.
...
The Notification API specification is currently under development, however the technology is supported by most of the modern browsers. Notifications consist of four main attributes: title, body, icon and tag (i.e. an identifier for the notification).
...
Currently, the notification system does not save updates from periods that the user was offline, however it addresses the manual checking issue by delivering useful and real-time information to the user.

%----------------------------------------------------------------------------
\subsection{Saved Messages}
\label{savedMessages}

The amount of messages in each chat can be considerable, so it can be quite difficult for the user to find important information in a chat. The user could just add a note to avoid this problem, however this means recreating what has already been done, as the user would have to find the relevant information and then copy and paste it to a new note in the chat. To solve this problem, the saved message feature was added. 

It consists of a small button [screenshot] located in the bottom right corner of each message in a chat. If pressed, the message will be saved as a metadata, in the tab information as shown below: [another screenshot]

With this feature, the user can have a list of all the relevant information of a chat and there is no need to read the entire conversation to understand what is happening. Thereby, there is no duplication of effort and every relevant content can be saved as the conversation is occurring.
...
\item An inclusion of a ‘newsfeed’ could greatly improve the usability of the site, allowing users to see what has been recently done on each project.  this can also replace the currently useless ‘profile page’.
...
As a result of the team using the application to manage their own project, they were able to evaluate the effectiveness of the tool.  As discussed in the previous section, the identification of key weaknesses allowed the team to fix these and improve the user experience as a whole.  As the team are part of the target market they were in the optimal position to evaluate the usefulness of the system as a whole.  With the inclusion of the additional features the team found the tool to be a wholly inclusive management system, while remaining easy to use.

%----------------------------------------------------------------------------
\section{Future Goals}
\label{futureGoals}

As a result of the prototype demonstration in December, due to feedback received an area of potential future development identified was integration with Jenkins.  This is a continuous integration system used to monitor and execute repeated jobs, such as building and testing software being developed.  This could be implemented through treating Jenkins as a user who would automatically create a ticket based on each build.  It could also automatically create a high priority ticket when a build fails, and tag the relevant developers.  This expansion would be possible due to Jenkins plugin based structure, whereby it is designed to be extended as necessary.

Another potential addition could be linking to Git and other source control platforms. Often, these systems support tickets and issue control to some degree.  For example, a conversation could be created automatically when a pull request is created on Github.  This could be used to discuss the changes being made and suggest possible modifications.
